...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Instance == TIM6) {
    ...
    float result0 =
        pid0.Update(static_cast<float>(round_speed + target_speed), speed0);
    // float result1 = pid1.Update(static_cast<float>(round_speed +
    // target_speed), speed1); float result2 =
    // pid2.Update(static_cast<float>(round_speed - target_speed), speed2);
    // float result3 = pid3.Update(static_cast<float>(-round_speed +
    // target_speed), speed3);
    ...
    // 发送电机速度
    /// @bug 发四次就炸了！！！
    auto sent_result = community.SendMotorSpeed(
        0x000, static_cast<int16_t>(speed0), target_speed, round_speed,
        static_cast<int16_t>(result0));
