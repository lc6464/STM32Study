...
void Encoder_Update(Encoder *encoder) {
  ...
  uint16_t currentCount =
      __HAL_TIM_GET_COUNTER(encoder->HTim); // 当前计数值，实际不会超过 65535
  uint32_t currentTime = HAL_GetTick(); // 获取当前时间（毫秒计时）
  uint8_t isTimeOverflow = currentTime < encoder->LastTime; // 判断是否时间溢出

  // 计算计数值位移
  int32_t diff = (int32_t)currentCount -
                 (int32_t)encoder->LastCount; // 计算当前计数与上次计数之差

  // 处理溢出情况
  diff += (int32_t)encoder->OverflowCount *
          (int32_t)(encoder->HTim->Instance->ARR + 1); // 增加溢出对应的计数值
  encoder->OverflowCount = 0;                          // 清零溢出计数

  // 计算时间差（毫秒）
  // uint32_t timeDiff = currentTime - encoder->LastTime; //
  // 被注释掉了，因为没有在下面的代码中用到
  ...
  if (diff == 0 && !encoder->IsStopped) {
    ...
    uint64_t timeSinceLastUpdate =
        (uint64_t)currentTime + ((uint64_t)isTimeOverflow * UINT32_MAX) -
        (uint64_t)encoder->LastUpdateTime; // 计算时间差距
  ...
  */
  ...
  // 计算速度（RPM）
  // float rawSpeed = (float)diff * 60000.0f / (float)(ENCODER_RESOLUTION *
  // GEAR_RATIO * timeDiff); // 原来用来计算速度的代码被注释掉
  float rawSpeed = (float)diff; // 当前实现中直接使用 diff 作为原始速度

  // 应用低通滤波
  encoder->Speed =
      (float)encoder->Speed * (1.0f - FILTER_ALPHA) +
      (float)rawSpeed * FILTER_ALPHA; // 低通滤波公式，计算出平滑速度
}
