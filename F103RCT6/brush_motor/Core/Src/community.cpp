...
HAL_StatusTypeDef Community::Transmit(CAN_TxHeaderTypeDef *txHeader, uint8_t *txData) {
	uint32_t mailbox;
	return HAL_CAN_AddTxMessage(_hcan, txHeader, txData, &mailbox);
}

HAL_StatusTypeDef Community::SendMotorSpeed(uint16_t canId, int16_t speed, int16_t target, int16_t round, int16_t pidOut) {
...
bool Community::ExecuteRxCallback(CAN_RxHeaderTypeDef *rxHeader, uint8_t *rxData) {
