 ...
 */
void Motor_Init(Motor *motor, TIM_HandleTypeDef *htim, const uint32_t channelA,
                const uint32_t channelB) {
 ...
 * @note 速度为正时, 电机正向旋转; 速度为负时, 电机反向旋转; 速度为 0 时,
 * 电机刹车
 * @return 设置状态
 */
Motor_StatusTypeDef Motor_SetSpeed(const Motor motor, const int16_t speed) {
  ...
  if (speed > 0 && speed <= 1000) {
    ...
    result.ChannelA =
        __HAL_TIM_SET_COMPARE(motor.HTim, motor.ChannelA, 1000 - speed);
    ...
    result.ChannelB =
        __HAL_TIM_SET_COMPARE(motor.HTim, motor.ChannelB, 1000 + speed);
  } else {
    // 刹车: 两个通道均为高电平
    result.ChannelA = __HAL_TIM_SET_COMPARE(motor.HTim, motor.ChannelA, 2000);
    result.ChannelB = __HAL_TIM_SET_COMPARE(motor.HTim, motor.ChannelB, 2000);
