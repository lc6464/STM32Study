/**
         ...
         *   - 板级支持层(Board Support
   Pack，BSP)：提供硬件驱动及接口，通常BSP层都是由厂家或者网上开源。
         *   - 硬件抽象层(Hardware Abstract
   Layer，HAL)：对BSP层进一步封装，形成统一的接口形式。
         ...
         *   - 系统库(System
   Library)：提供诸如内存管理、任务间通讯、外部通讯、调试等功能。
         *   - 解算库(Solution
   Library)：提供解算算法，如陀螺仪解算、底盘运动模型解算、功率限制等。
         *   - 通讯库(Communications
   Library)：提供电机通讯、裁判系统解算、多机通讯等。
         * - 应用层(Application)
