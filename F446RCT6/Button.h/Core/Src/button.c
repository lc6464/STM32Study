...
#error Button in the sampling time is less than 1.(button.c)
  ...
  if (button_level == button->active_level)
    button->press_count =
        (button->press_count >= 2 * (Button_Sampling_Window)-1)
            ? 2 * (Button_Sampling_Window)-1
            : button->press_count + 1;
  else
    button->press_count =
        (button->press_count <= 0) ? 0 : button->press_count - 1;
  // 确认按键状态,开始处理
  if (button->press_count >= Button_Sampling_Window) {
    ...
    case 1:
      // 按键按着
      if (Button_GetTick() - button->press_down_time >
    ...
    case 2:
      // 等待再按下超时
      if (Button_GetTick() - button->press_up_time > Button_Clink_Timeout &&
          button->press_up_time != 0) {
        // 长按和点击互斥
        if (!button->long_press_flag) {
