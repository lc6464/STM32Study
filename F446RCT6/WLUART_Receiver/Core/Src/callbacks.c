...
extern "C"
{
    ...
    void USART1_ReceiveProcessor(uint32_t data_length)
    {
        ...
        {
            HAL_UART_Transmit_DMA(&huart1, (uint8_t *)"Invalid", 7); // 向 USART1 报告数据无效
            return;
        }
        ...
        HAL_I2C_Mem_Write(&hi2c2, SSD1306_I2C_ADDR, 0x00, 1, commands, 3, 15); // 不能连续用 DMA，否则下一个会报 Busy，懒得写队列了
        HAL_I2C_Mem_Write_DMA(&hi2c2, SSD1306_I2C_ADDR, 0x40, 1, &SSD1306_Buffer[SSD1306_WIDTH * 3], SSD1306_WIDTH * 2);

        HAL_UART_Transmit_DMA(&huart2, uart_receive_buffer, data_length); // 向 USART2 转发数据
    }
