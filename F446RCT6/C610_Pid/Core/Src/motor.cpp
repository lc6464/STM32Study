...
void set_motor_current(uint8_t id_range, int16_t v1, int16_t v2, int16_t v3, int16_t v4) {
	CAN_TxHeaderTypeDef tx_header = {
		.StdId = id_range == 0 ? CAN_CONTROL_ID_BASE : CAN_CONTROL_ID_EXTEND,
		.ExtId = 0,
		.IDE = CAN_ID_STD,
		.RTR = CAN_RTR_DATA,
		.DLC = 8,
	...
	uint8_t tx_data[8] = {
		...
		static_cast<uint8_t>(v2 >> 8),
		static_cast<uint8_t>(v2),
		static_cast<uint8_t>(v3 >> 8),
		static_cast<uint8_t>(v3),
		static_cast<uint8_t>(v4 >> 8),
		static_cast<uint8_t>(v4)
	};
