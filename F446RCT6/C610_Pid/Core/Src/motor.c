 ...
 * @retval None
 */
void set_motor_current(uint8_t id_range, int16_t v1, int16_t v2, int16_t v3,
                       int16_t v4) {
...
void LC_CAN1_Config(void) {
  ...
  can_filter.FilterMode =
      CAN_FILTERMODE_IDMASK; // 设置过滤器模式为标识符掩码模式
  ...
  can_filter.SlaveStartFilterBank =
      14; // 在双 CAN 模式下有意义，设置从过滤器起始编号为 14

  HAL_CAN_ConfigFilter(&hcan1, &can_filter); // 配置 CAN1 的过滤器
  HAL_CAN_Start(&hcan1);                     // 启动 CAN1
  HAL_CAN_ActivateNotification(
      &hcan1, CAN_IT_RX_FIFO0_MSG_PENDING); // 使能 CAN1 接收 FIFO0 消息挂起中断
}