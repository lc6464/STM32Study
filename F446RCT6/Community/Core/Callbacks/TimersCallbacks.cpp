...
void MapRemoteToMotorSpeed(int16_t remoteX, int16_t remoteY,
  ...
  const float maxTargetSpeed =
      (mode == SpeedMode::Low) ? LOW_SPEED_MAX : HIGH_SPEED_MAX;

  // 限制遥控器输入范围
  remoteX = std::clamp(remoteX, static_cast<int16_t>(-MAX_REMOTE_VALUE),
                       static_cast<int16_t>(MAX_REMOTE_VALUE));
  remoteY = std::clamp(remoteY, static_cast<int16_t>(-MAX_REMOTE_VALUE),
                       static_cast<int16_t>(MAX_REMOTE_VALUE));
  ...
  // 应用平滑过渡
  leftMotorSpeed =
      lastLeftMotorSpeed * (1 - SMOOTH_FACTOR) + leftMotorSpeed * SMOOTH_FACTOR;
  rightMotorSpeed = lastRightMotorSpeed * (1 - SMOOTH_FACTOR) +
                    rightMotorSpeed * SMOOTH_FACTOR;
