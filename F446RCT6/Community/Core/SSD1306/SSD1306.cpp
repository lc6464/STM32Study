...
HAL_StatusTypeDef SSD1306::Start() {
  ...
  status +=
      WriteCommand(0x10); // 发送命令：设置为页面寻址模式 (00=水平寻址模式,
...
void SSD1306::UpdateScreen() {
  for (uint8_t i = 0; i < 8; i++) {
    ...
    HAL_I2C_Mem_Write(_hi2c, I2C_ADDR, 0x40, 1, _buffer.data() + (WIDTH * i),
                      WIDTH, 100);
  }
}
...
char SSD1306::WriteChar(char ch, SSD1306_Fonts Font,
...
char SSD1306::WriteString(const char *str, SSD1306_Fonts Font,
                          Color color /* = Color::White*/) {
  // 写入直到空字节
  while (*str) {
