...
HAL_StatusTypeDef SSD1306::Start() {
  ...
  status +=
      WriteCommand(0x00); // 发送命令：设置为页面寻址模式 (00=水平寻址模式,
                          // 01=垂直寻址模式, 10=页面寻址模式, 11=无效)
  ...
  status += WriteCommand(0xA4); // 发送命令：关闭整个显示 (0xA4=显示跟随 RAM
                                // 内容, 0xA5=忽略 RAM 内容，全部点亮)
  ...
  status += WriteCommand(COM_LR_REMAP << 5 | COM_ALTERNATIVE_PIN_CONFIG << 4 |
                         0x02); // 设置 COM 引脚硬件配置的具体值
...
HAL_StatusTypeDef SSD1306::WriteCommand(uint8_t command) {
  return HAL_I2C_Mem_Write(_hi2c, I2C_ADDR, 0x00, I2C_MEMADD_SIZE_8BIT,
                           &command, 1, 10);
}
...
void SSD1306::UpdateScreen() {
  HAL_I2C_Mem_Write_DMA(_hi2c, I2C_ADDR, 0x40, I2C_MEMADD_SIZE_8BIT,
...
char SSD1306::WriteChar(char ch, SSD1306Font Font,
                        Color color /* = Color::White*/) {
  // 检查当前行剩余空间
  if (WIDTH <= (_currentX + Font.FontWidth) ||
      HEIGHT <= (_currentY + Font.FontHeight)) {
    // 当前行空间不足
    return 0;
  }
  ...
  for (uint32_t i = 0; i < Font.FontHeight; i++) {
    ...
    for (uint32_t j = 0; j < Font.FontWidth; j++) {
      if ((b << j) & 0x8000) {
        ...
        DrawPixel(_currentX + j, (_currentY + i),
                  static_cast<Color>(static_cast<uint8_t>(color) ^ 1));
      }
    }
  }
...
char SSD1306::WriteString(const char *str, SSD1306Font Font,
                          Color color /* = Color::White*/) {
  // 写入直到空字节
  while (*str) {
...
void SSD1306::InvertColors() { _inverted = !_inverted; }