...
class Mailboxes {
private:
  static std::vector<std::unique_ptr<uint32_t>>
      _mailboxes; // 使用静态 vector 存储邮箱指针
...
public:
  ...
  // 获取当前邮箱数量
  static size_t GetCount() { return _mailboxes.size(); }
...
private:
  ...
  static void _cancelMailboxTransmission(uint32_t mailboxValue) {
    HAL_CAN_AbortTxRequest(&hcan2, mailboxValue);
  }
};
