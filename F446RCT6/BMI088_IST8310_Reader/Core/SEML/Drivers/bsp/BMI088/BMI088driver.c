...
static void BMI088_write_single_reg(SPI_Handle_t *handle, uint8_t reg, uint8_t data)
{
	BMI088_read_write_byte(handle, reg);
	BMI088_read_write_byte(handle, data);
}

static void BMI088_read_single_reg(SPI_Handle_t *handle, uint8_t reg, uint8_t *return_data)
{
	BMI088_read_write_byte(handle, reg | 0x80);
	*return_data = BMI088_read_write_byte(handle, 0x55);
}

static void BMI088_read_muli_reg(SPI_Handle_t *handle, uint8_t reg, uint8_t *buf, uint8_t len)
{
	BMI088_read_write_byte(handle, reg | 0x80);
...
uint8_t BMI088_Accel_Init(BMI088_Data_t *bmi088)
{
	...
	BMI088_accel_write_single_reg(bmi088, BMI088_ACC_SOFTRESET, BMI088_ACC_SOFTRESET_VALUE);
	...
	for (write_reg_num = 0; write_reg_num < BMI088_WRITE_ACCEL_REG_NUM; write_reg_num++)
	{
		...
		BMI088_accel_write_single_reg(bmi088, write_BMI088_accel_reg_data_error[write_reg_num][0], write_BMI088_accel_reg_data_error[write_reg_num][1]);
		BMI088_Delay(1);

		BMI088_accel_read_single_reg(bmi088, write_BMI088_accel_reg_data_error[write_reg_num][0], res);
		BMI088_Delay(1);
...
uint8_t BMI088_Gyro_Init(BMI088_Data_t *bmi088)
{
	...
	BMI088_gyro_write_single_reg(bmi088, BMI088_GYRO_SOFTRESET, BMI088_GYRO_SOFTRESET_VALUE);
	BMI088_Delay(BMI088_LONG_DELAY_TIME);
	// check commiunication is normal after reset
	BMI088_gyro_read_single_reg(bmi088, BMI088_GYRO_CHIP_ID, res);
	BMI088_Delay(1);
	...
	for (write_reg_num = 0; write_reg_num < BMI088_WRITE_GYRO_REG_NUM; write_reg_num++)
	{
		...
		BMI088_gyro_write_single_reg(bmi088, write_BMI088_gyro_reg_data_error[write_reg_num][0], write_BMI088_gyro_reg_data_error[write_reg_num][1]);
		BMI088_Delay(1);

		BMI088_gyro_read_single_reg(bmi088, write_BMI088_gyro_reg_data_error[write_reg_num][0], res);
		BMI088_Delay(1);
