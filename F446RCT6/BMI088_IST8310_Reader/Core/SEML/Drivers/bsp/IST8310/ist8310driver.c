...
#define MAG_SEN                                                                \
  0.3f // raw int16 data change to uT unit. 原始整型数据变成 单位ut
...
// the second column: the value to be writed to the
// registers.第二列:需要写入的寄存器值 the third column: return error
// value.第三列:返回的错误码
static const uint8_t ist8310_write_reg_data_error[IST8310_WRITE_REG_NUM][3] = {
    {0x0B, 0x08,
     0x01}, // enalbe interrupt  and low pin polarity.开启中断，并且设置低电平
...
uint8_t ist8310_init(ist8310_Handle_t *ist8310_handle) {
  ...
  SEML_I2C_Mem_Read(&ist8310_handle->I2C_Handle, IST8310_IIC_ADDRESS << 1,
                    IST8310_WHO_AM_I, I2C_MEMADD_SIZE_8BIT, &res, 1, 10);
#else
  ist8310_RST_L();
  ...
  for (writeNum = 0; writeNum < IST8310_WRITE_REG_NUM; writeNum++) {
    ...
    SEML_I2C_Mem_Write(&ist8310_handle->I2C_Handle, IST8310_IIC_ADDRESS << 1,
                       ...
                       I2C_MEMADD_SIZE_8BIT, &temp, 1, 10);
#else
...
#ifdef USE_SEML_LIB
  SEML_I2C_Mem_Read(&ist8310_handle->I2C_Handle, IST8310_IIC_ADDRESS << 1, 0x03,
