...
* @brief      the file provide I2C write/read function, as the middleware of
IST8310.
...
uint8_t ist8310_IIC_read_single_reg(uint8_t reg) {
  uint8_t res = 0;
  HAL_I2C_Mem_Read(&hi2c3, IST8310_IIC_ADDRESS << 1, reg, I2C_MEMADD_SIZE_8BIT,
                   &res, 1, 10);
  return res;
}
 ...
 * @param[in]      register ad vv   b  n nmmmdress
...
void ist8310_IIC_write_single_reg(uint8_t reg, uint8_t data) {
  HAL_I2C_Mem_Write(&hi2c3, IST8310_IIC_ADDRESS << 1, reg, I2C_MEMADD_SIZE_8BIT,
                    &data, 1, 10);
}
 ...
 * @retval         none
 */
void ist8310_delay_ms(uint16_t ms) { HAL_Delay(ms); }
