 ...
 * @return x mod y结果
 */
float math_fmod(float x, float y) { return x - (int)(x / y) * y; }
 ...
 * @return e^x的结果
 */
float math_exp(float x) { return exp(x); }
 ...
 * @return log_a(x)结果
 */
float math_log(float a, float x) { return math_ln(x) / math_ln(a); }
 ...
 * @return 1/sqrt(x)结果
 */
float math_invsqrt(float x) { return 1 / math_sqrt(x); }
