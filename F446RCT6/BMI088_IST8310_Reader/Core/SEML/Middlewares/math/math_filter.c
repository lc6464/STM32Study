...
void RMS_filter_Init(RMS_filter_t *RMS_filter, const uint16_t window_width,
  ...
  if (InitQueue(&RMS_filter->data_queue, sizeof(float), window_width, buffer,
                queue_full_hander_error) == queue_no_error) {
...
void average_filter_Init(average_filter_t *ave_filter,
  ...
  if (InitQueue(&ave_filter->data_queue, sizeof(float), window_width, buffer,
                queue_full_hander_error) == queue_no_error) {
    for (i = 0; i < window_width; i++)
