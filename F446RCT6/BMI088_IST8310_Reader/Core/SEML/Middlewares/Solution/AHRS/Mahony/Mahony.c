...
void Mahony_AHRS_Update(float quat[4], float sample_time, Accel_Data_t *accel,
  ...
  // accelerometer normalisation) 只在加速度计数据有效时才进行运算
  if (accel != NULL)
    if (!(IS_NAN_INF(accel_.x) || IS_NAN_INF(accel_.y) ||
      ...
      // Compute feedback only if magnetometer measurement valid (avoids NaN in
      // magnetometer normalisation) 只在磁力计数据有效时才进行运算
      if (mag != NULL)
