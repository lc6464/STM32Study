...
float Xoffest = (0.0 - 66.6) / 2.0,
Yoffest = (34.5 - 30.0) / 2.0,
Kx = 2.0 / (0.0 + 66.6),
Ky = 2.0 / (34.5 + 30.0);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Instance == TIM6) {
    ...
    float x = ist8310_data.Data.X,
      y = ist8310_data.Data.Y,
      z = ist8310_data.Data.Z,
      magAngle = 0.0f;

    x = (x - Xoffest) * Kx;
    x = (y - Yoffest) * Ky;

    if ((x > 0) && (y > 0)) magAngle = atan(y / x) * 57;
    else if ((x > 0) && (y < 0)) magAngle = 360 + atan(y / x) * 57;
    else if ((x == 0) && (y > 0)) magAngle = 90;
    else if ((x == 0) && (y < 0)) magAngle = 270;
    else if (x < 0) magAngle = 180 + atan(y / x) * 57;
    ...
    uint8_t zLength =
      floatToString(z, xyz + 3 + xLength + 4 + yLength + 4);
    strcpy(xyz + 3 + xLength + 4 + yLength + 4 + zLength, "\nAngle: ");
    uint8_t angleLength =
      floatToString(magAngle, xyz + 3 + xLength + 4 + yLength + 4 + zLength + 7);
    strcpy(xyz + 3 + xLength + 4 + yLength + 4 + zLength + 7 + angleLength, "\n");

    HAL_UART_Transmit_DMA(&huart1, (uint8_t *)xyz,
      3 + xLength + 4 + yLength + 4 + zLength + 7 + angleLength + 1);
  }
}
...
int main(void) {
  ...
  uint8_t minXLength = floatToString(minX, mxyz + 6 + xLength + 8 + yLength + 8 + zLength + 8);
  strcpy(mxyz + 6 + xLength + 8 + yLength + 8 + zLength + 8 + minXLength, "\nMin Y: ");
  uint8_t minYLength = floatToString(minY, mxyz + 6 + xLength + 8 + yLength + 8 + zLength + 8 + minXLength + 8);
  strcpy(mxyz + 6 + xLength + 8 + yLength + 8 + zLength + 8 + minXLength + 8 + minYLength, "\nMin Z: ");
  uint8_t minZLength = floatToString(minZ, mxyz + 6 + xLength + 8 + yLength + 8 + zLength + 8 + minXLength + 8 + minYLength + 8);
  strcpy(mxyz + 6 + xLength + 8 + yLength + 8 + zLength + 8 + minXLength + 8 + minYLength + 8 + minZLength, "\n");

  HAL_UART_Transmit_DMA(&huart1, (uint8_t *)mxyz,
    6 + xLength + 8 + yLength + 8 + zLength + 8 + minXLength + 8 + minYLength + 8 + minZLength + 1);
