...
inline bool is_nan_or_inf(float x) {
	// 检查 x 是否为 NaN（非数字）或无穷大
 ...
 */
template<typename T>
constexpr auto low_pass_filter(T current_value, T previous_average, float smoothing_factor) noexcept
 ...
 * @return 姿态四元数数据数组
 */
static inline const float *Get_AHRS_Quat(const AHRS_t *AHRS) {
	return AHRS->quat;
}
